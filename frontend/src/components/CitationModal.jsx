...
// is opened by CitedView / DefinitionPanel when the user clicks a
// normalized [/facts/{id}] or [/concepts/{id}] link, so reading a
// cited fact does not disrupt the surrounding synthesis/report.
//
...
//     (for image facts), and a "View full fact page" link.
...
export default function CitationModal(props) {
  ...
  const fact = () => (data()?.kind === "fact" ? data().fact : null);
  const concept = () => (data()?.kind === "concept" ? data().concept : null);
    ...
    <Modal open={props.open} onClose={props.onClose} title={title()}>
      <Suspense fallback={<Loading message="Loading..." />}>
        <Show when={!data.loading} fallback={<Loading message="Loading..." />}>
          <Show when={data()} fallback={<p class="text-text-muted italic">Not found.</p>}>
            {/* ── FACT ── */}
            <Show when={fact()}>
              {(f) => (
                <div class="space-y-3">
