...
export default function ChainOverview(props) {
  return (
    <Card class="mb-6">
      <h2 class="text-lg font-semibold mb-1 dark:text-white">Fetch chain overview</h2>
      <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
        How a source URL or DOI is resolved into a fetched body. Providers are tried top-to-bottom;
        the first match wins. The chain is built at server startup and reflects the current
        configuration.
      </p>
      <Show
          ...
          <For each={props.providers()}>
            {(p, i) => (
              <li class="flex items-center gap-3 text-sm">
                ...
                <span class="font-mono text-xs text-gray-400 dark:text-gray-500">{p.id}</span>
                <Show
                  when={p.configured}
                  fallback={
                    <span class="text-xs text-amber-600 dark:text-amber-400">not configured</span>
                  }
                >
