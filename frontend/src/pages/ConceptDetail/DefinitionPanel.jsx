import { Show, createResource, createSignal, createEffect, onCleanup, untrack } from "solid-js";
    ...
    <Card>
      <div class="flex items-center justify-between gap-3 flex-wrap">
        ...
        >
          ...
          <span class="text-xs text-gray-400 dark:text-gray-500 transition-transform group-hover:text-gray-600 dark:group-hover:text-gray-300" style={collapsed() ? "transform: rotate(-90deg)" : ""}>▾</span>
        </button>
        <button
          type="button"
        ...
        <div id="concept-definition-body" class="mt-4">
          <Show
            ...
            fallback={
              <Show
                when={notFound()}
                fallback={<EmptyState title="Couldn't load the definition." description={defData()?.error?.message || "Something went wrong fetching the definition. Try refreshing."} />}
              >
                <EmptyState title="No definition yet." description="The definition is generated by the synthesize_concept task once enough summaries exist for this concept. Check back after the next synthesis pass." />
              </Show>
            }
          >
            <Show when={synthesis()}>
              <div class="border rounded dark:border-gray-700 p-4">
                <div class="flex items-center gap-2 mb-3 flex-wrap">
                  <Show when={synthesis().model}>
                    <span class="text-xs text-gray-400 dark:text-gray-500">{synthesis().model}</span>
                  </Show>
                  <Show when={synthesis().updated_at}>
                    <span class="text-xs text-gray-400 dark:text-gray-500">updated {new Date(synthesis().updated_at).toLocaleString()}</span>
                  </Show>
                  <Show when={images().length > 0}>
                    <Badge variant="gray">{images().length} image{images().length === 1 ? "" : "s"}</Badge>
                  </Show>
                </div>
                <div
                  class="prose dark:prose-invert max-w-none text-sm text-gray-800 dark:text-gray-200 leading-relaxed"
                  ref={(el) => { defEl = el; if (el) el.innerHTML = renderedHtml(); }}
