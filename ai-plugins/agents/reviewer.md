<!--
  ...
  .opencode/agent/reviewer.md by ai-plugins/scripts/sync-agents.mjs.
...
|---|----------|----------------|-----|
| 1a | **Citation reality** | Every `<fact:ID>` and `<concept:ID>` (and bare `<uuid>`) in the doc resolves to a real fact/concept in the graph. | Regex-extract every UUID from the markdown — prefixed (`fact:`, `concept:`), bare (`<uuid>`), and image (`![alt](<fact:uuid>)`). Batch `getFact` / `getConcept`. Flag 404s, wrong-kind IDs (a `concept:` ID that is actually a fact UUID — note the system auto-corrects this but it signals prompt-following failure), malformed links, and hallucinated UUIDs. |
| 1b | **Citation syntax** | Every `fact:` / `concept:` token is wrapped in `<>` angle brackets inside a markdown link `[...](<...>)` or image embed `![...](<...>)`. Bare-bracket forms (`[fact:uuid]`, `[concept:uuid]`) and unbracketed link targets (`[text](fact:uuid)`) render as literal text in the UI — they are critical syntax failures, not cosmetic. | Regex-scan for every `fact:` / `concept:` token in the markdown. For each token, verify it sits inside `<>` and inside a `[...](<...>)` or `![...](<...>)` construct. Flag any `fact:` / `concept:` token NOT wrapped in `<>` as a "Citation syntax failure — will not render in UI" (critical, separate from the UUID-exists check in 1a). Suggested fix: rewrite as `[description](<fact:uuid>)` / `[name](<concept:uuid>)` / `![alt](<fact:uuid>)`. |
...
3. **Meta-synthesis mode only**: also read every sub-synthesis file the
 ...
 4. **Extract citations** — regex-extract every UUID from the markdown:
  ...
  5b. **Verify citation syntax** — for every `fact:` / `concept:` token
     found in step 4, verify it sits inside `<>` and inside a
     `[...](<...>)` or `![...](<...>)` construct. Flag any token NOT
     wrapped in `<>` (e.g. `[fact:uuid]`, `[concept:uuid]`,
     `[text](fact:uuid)`) as a citation syntax failure — it will render
     as literal text in the UI. This is a critical failure, separate from
     the UUID-exists check in step 5.
...
## Citation Syntax Failures
### [section heading where the token appears]
- Token: `[fact:ID]` (no angle brackets) / `[concept:ID]` (no angle brackets) / `[text](fact:ID)` (no angle brackets)
- Failure: will NOT render in the UI — the token appears as literal text
- Suggested fix: rewrite as `[description](<fact:ID>)` / `[name](<concept:ID>)` / `![alt](<fact:ID>)`
