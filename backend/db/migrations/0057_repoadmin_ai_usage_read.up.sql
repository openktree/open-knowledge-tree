-- 0057_repoadmin_ai_usage_read.up.sql
--
-- Backfills the repoadmin -> ai_usage.read policy into existing
...
-- INSERT to grant repoadmin the ai_usage.read permission that the
-- new repo-scoped AI Usage feature expects. The policy is
-- repo-scoped at enforcement time (RequireRepoPermission reads the
-- repoID from the URL), so the row uses "*" as the domain -- the
-- same form seed.go uses.
--
...
SELECT 'p', 'repoadmin', '*', 'ai_usage', 'read'
WHERE NOT EXISTS (
