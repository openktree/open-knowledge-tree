...
// retryConfig governs the synthesis provider's retry-with-backoff
// loop. The defaults mirror summarization.defaultRetryConfig so the
// synthesis provider behaves the same as the other AI-backed
// providers against OpenRouter / Ollama Cloud: 429 + 5xx + network
// errors are retried; 4xx (other than 429) is permanent.
...
// retrying. Mirrors summarization.classifyError so the synthesizer
// rides out the same transient failures.
...
// for plain markdown. The synthesizer treats the response as the
// synthesis body, so any wrapper must be removed before persisting.
